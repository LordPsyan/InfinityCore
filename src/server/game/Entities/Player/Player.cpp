...
void Player::UpdateZone(uint32 newZone)
{
    ...
    if (zone->flags & AREA_FLAG_SANCTUARY || GetAreaId() == 876|| GetAreaId() == 2037)                   // in sanctuary
    {
    ...
    for (GossipMenuItemsMap::const_iterator itr = pMenuItemBounds.first; itr != pMenuItemBounds.second; ++itr)
    {
        ...
        if (pSource->GetTypeId() == TYPEID_UNIT)
        {
            ...
            case GOSSIP_OPTION_VENDOR:
                {
                    VendorItemData const* vItems = itr->second.action_menu_id ? nullptr : pCreature->GetVendorItems();
                    if (!itr->second.action_menu_id && (!vItems || vItems->Empty()))
                    {
...
bool Player::BuyItemFromVendor(uint64 vendorguid, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    VendorItemData const* vItems = currentVendor ? sObjectMgr.GetNpcVendorItemList(currentVendor) : pCreature->GetVendorItems();
